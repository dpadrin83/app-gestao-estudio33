...
- **Tipo de projeto "Consultoria" (projeto guarda-chuva recorrente) — A FEATURE GRANDE, ainda a planejar:** falta o **comportamento** de consultoria, que deve: (1) ser **recorrente** — valor **mensal**, com **data de início** e **sem fim fixo** (ex.: anual renovável), em vez de valor fechado único; (2) funcionar como **guarda-chuva** que vai recebendo vários entregáveis ao longo do tempo (identidade → site → sistema → demandas pontuais), tudo virando **um grande cronograma**; (3) ter uma **etapa inicial de onboarding** comum a todos os projetos (cadastro do cliente + coleta de briefing) antes de iniciar. O Danilo quer poder rodar **tanto** projeto pontual (uma área) **quanto** esse macro. 1º caso real: cliente músico/percussionista (projeto "Construção da Marca"). **Já feito como passo interino (2026-05-28):** adicionada a opção **"Consultoria"** (valor `consulting`) ao campo de tipo de serviço — código (enum + label) + migration `20260528120000_add_consulting_service_line.sql`. Isso é **só o rótulo na lista**; o comportamento recorrente/guarda-chuva acima continua pendente.
- **Rótulo do campo de serviço:** renomeado de "Área E33" → **"Tipo de serviço"** ([src/components/forms/project-form.tsx](src/components/forms/project-form.tsx)). As opções já estão em PT-BR no código (`src/lib/format.ts` → `serviceLineLabels`). Se aparecer em inglês na tela (ex.: "hybrid"), é versão publicada **desatualizada** — recarregar local / republicar resolve.
